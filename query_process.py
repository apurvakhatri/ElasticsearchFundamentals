...
def getTermVector(term):
    ...
    result = es.search(index=es_index,
                       ...
                       body={
                           ...
                           "size": 10000,
                           "_source": {
                               "includes": ["_id"]  # Specify fields to include
                           }
                       })
...
def okapiBM25(term_freq_dict, avg_doc_length, queryNum, term_DF_dict, total_docs, queryArray):
    #print("term DF dict: ", term_DF_dict)
    okapiBM25Score = {}
    k1 = 1.2
    k2 = 100
    b = 0.75
    ...
    for doc_id in term_freq_dict.keys():
        ...
        for term in term_freq_dict[doc_id][1].keys():
            ...
            #print("term_DF_dict[term]: ", term_DF_dict[term])
            if term in term_DF_dict:
                #print("Term is: ", term)
                #print("term df dict: ", term_DF_dict)
                new_df = term_DF_dict[term]["value"] + 0.5
            else:
                new_df = 0.5
...
def unigramLMLaplace(term_freq_dict, queryNum, vocab_size, stemmedArray):
    ...
    rank = 1
    out = open("unigramLMLaplace.txt", "a")
...
def unigramLMJelMer(term_freq_dict, queryNum, vocab_size, stemmedArray, term_TTF_dict, doc_length_dict):
    ...
    rank = 1
    out = open("unigramLMJelinekMercer.txt", "a")
